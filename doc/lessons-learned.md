...
|---|---|---|---|
| 2026-04-24 | KB document list showed Cyrillic filenames as `______11_12_2020_N_887__.txt` | N8N sanitizes multipart `filename` to ASCII before storing in `kb_documents.title`; original Unicode name was discarded | `_fix_doc_meta()` now overwrites `title` + stores `structure.preview` via direct Postgres UPDATE after every N8N ingest; `_kb_list_documents_direct()` returns `preview` from structure JSONB |
| 2026-04-24 | After KB extraction fix, old RTF doc record remained with 0 chunks — search still returned nothing | Fixed code was deployed but stale data from the broken ingest (0-chunk document) was never cleaned up or re-ingested | Added mandatory rule: fix to agent/service → always migrate/clean existing data. User must delete stale records and re-upload. |
