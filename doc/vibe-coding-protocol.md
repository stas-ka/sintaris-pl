...
| ~14:00 UTC | Worksafety KB quality analysis: analyzed Worksafety N8N RAG bot architecture, compared with Taris KB (384-dim vs 1536-dim, scoring formulas, query types, chapter metadata), created doc/kb-quality-comparison.md (architecture table, gap analysis, 10 benchmark queries, improvement roadmap P1-P3), added T233-T241 to test_remote_kb.py (T233-T239 offline, T240-T241 live benchmark), updated doc/test-suite.md KB row T200-T241. 185/185 baseline PASS. | 3 | ~8 | claude-sonnet-4.6 | doc/kb-quality-comparison.md, src/tests/test_remote_kb.py, doc/test-suite.md | done |
| ~16:00 UTC | Error Observer Agent: new bot_error_observer.py (ErrorCaptureHandler, _error_buffer, start/stop_observer, write_error_to_git_dir, make_description_md, update_errors_index); bot_error_protocol.py git sync integration; bot_config.py +3 constants (GIT_ERRORS_DIR, ERROR_GIT_AUTO_PUSH, LOG_FILE); strings.json errp_git_pushed/errp_git_push_fail; telegram_menu_bot.py start_observer in main(); errors/README.md; .github/skills/taris-error-review/SKILL.md; T242-T245 (20/20 PASS); doc/test-suite.md §2.5c; copilot-instructions.md skill row. 185/185 pytest PASS. | 3 | ~15 | claude-sonnet-4.6 | src/features/bot_error_observer.py, src/features/bot_error_protocol.py, src/core/bot_config.py, src/strings.json, src/telegram_menu_bot.py, src/tests/test_voice_regression.py, errors/README.md, .github/skills/taris-error-review/SKILL.md, doc/test-suite.md, .github/copilot-instructions.md | done |
| ~13:55 UTC | Deploy v2026.4.76 to VPS-Supertaris: deployed bot_config.py, bot_error_observer.py, bot_error_protocol.py, telegram_menu_bot.py, strings.json, release_notes.json + test file to Docker volume; added GIT_ERRORS_DIR and ERROR_GIT_AUTO_PUSH=0 to /opt/taris-docker/bot.env; restarted containers; verified Version: 2026.4.76 + Polling Telegram; T242-T245 20/20 PASS on VPS, 765 PASS total (was 715). TS2+TS1 unreachable (offline). | 2 | ~5 | claude-sonnet-4.6 | VPS: /opt/taris-docker/app/src/ (deploy only) | done |
...
| 22:00 UTC | run full tests on VPS (offline 136 pass; voice 719/7-pre-existing; Playwright 101 pass); fix: admin menu widget count 13→14/15; fix: _api_url strips ROOT_PATH for external VPS URLs (T_CW_01/08/11/12 were 404); diagnose stas/buerger wrong credentials → found admin/admin works | 2 | 8 | claude-sonnet-4.6 | src/tests/screen_loader/test_screen_loader.py, src/tests/ui/test_ui.py | done |
...
| 18:35 UTC | KB eval + regression: (1) T228-T232 regression tests (fastembed not Ollama, single-string embed call, LLM answer not raw chunks, _fix_doc_meta called); (2) eval_kb_vs_worksafety.py — 10-question comparison taris KB vs N8N Hybrid RAG via HTTP; baseline run shows N8N wins 7/10 on recall (full KB) vs taris 32% (empty KB — content missing, hallucinating from training); (3) deployed tests to VPS Docker, all PASS | 3 | 12 | claude-sonnet-4.6 | src/tests/test_remote_kb.py, tools/eval_kb_vs_worksafety.py, doc/test-suite.md | done |
...
| 23:05 UTC | VPS deploy §4.3: taris_kb DB created (6 tables), parse_doc.py→/opt/taris-mcp-kb/, docling+pdfminer.six+python-docx+mcp installed, all source files deployed to ~/.taris/, fixed User=stas in service files (systemd GROUP 216 bug), REMOTE_KB_ENABLED=0 in bot.env pending N8N setup, service active v2026.4.75, committed a86d571 | 2 | 1 | claude-sonnet-4.6 | ~/.taris/core/bot_config.py, ~/.taris/core/bot_mcp_client.py, ~/.taris/core/device_variant.py, ~/.taris/features/bot_remote_kb.py, ~/.taris/telegram_menu_bot.py, ~/.taris/bot_web.py, ~/.taris/strings.json, ~/.taris/bot.env, ~/.config/systemd/user/taris-telegram.service, src/services/taris-telegram.service, src/services/taris-web.service, deploy/system-configs/vps/mcp-kb/init_taris_kb.sql | done |
| 21:25 UTC | Fix VPS Docker deploy: files deployed to wrong path (~/.taris/ instead of /opt/taris-docker/app/src/); re-deployed all §4.3 files to Docker; appended MCP/KB env block to /opt/taris-docker/bot.env; restarted taris-telegram+taris-web containers; verified v2026.4.75 Polling Telegram…; fixed UI tests (PICO_BASE_URL must be http://localhost:8090 not …/supertaris-vps — ROOT_PATH is nginx-only); 84/90 tests pass (6 pre-existing failures) | 2 | 8 | claude-sonnet-4.6 | /opt/taris-docker/app/src/ (7 files), /opt/taris-docker/bot.env, src/tests/ui/conftest.py | done |
...
| Time (UTC) | Request | Complexity | Requests | Model | Files changed | Status |
|---|---|---|---|---|---|---|
| 13:45 | Fix Copilot OAuth session injection: code-server's context.secrets proxies to browser localStorage, not gnome-keyring; patched getToken() in extension.js to read /home/stas/.copilot-token.json first (file-based fallback); decrypted 3 OAuth sessions from local VS Code state.vscdb (AES-256-GCM + DPAPI key); verified all 3 tokens against copilot_internal/v2/token → HTTP 200; Node.js simulation confirms PATCH WILL WORK CORRECTLY when browser connects | 5 | 25 | claude-sonnet-4.6 | VPS: extension.js (2 patches: client ID + getToken), /home/stas/.copilot-token.json | done |
| 10:44 | Fix Copilot 404 in code-server: root cause = extension.js had hardcoded code-server OAuth app `01ab8ac9400c4e429b23` (not trusted by Copilot API); patch extension.js `Gt.gitHubClientId` to `Iv1.b507a08c87ecfe98` (VS Code app, trusted); also fix product.json `gitHubAuthentication.clientId`; clear keyring; restart code-server. | 4 | 12 | claude-sonnet-4.6 | VPS: /usr/lib/code-server/lib/vscode/extensions/github-authentication/dist/extension.js, /usr/lib/code-server/lib/vscode/product.json | done |
| 09:54 | Diagnose Copilot empty model list: GitHub account stas-ka has no Copilot subscription (404 on /user/copilot API); install Continue extension (linux-arm64 VSIX) as alternative; configure with OpenAI key from taris Docker container | 2 | 8 | claude-sonnet-4.6 | VPS: /home/stas/.continue/config.json, /home/stas/.local/share/code-server/extensions/continue.continue | done |
| 09:38 | Fix gnome-keyring locked collection: kill competing keyring instances, create systemd user override with --unlock + empty password, simplify code-server wrapper to use persistent /run/user/1000/bus D-Bus session | 3 | 6 | claude-sonnet-4.6 | /usr/local/bin/code-server-wrapper.sh, /usr/local/bin/keyring-starter.sh, ~/.config/systemd/user/gnome-keyring-daemon.service.d/unlock.conf (VPS only) | done |
...
| 12:30 UTC | Deploy v2026.4.67: deploy bot_notify to VPS docker; discovered+fixed contacts.telegram missing in DB schema (bot_db.py, store_postgres.py, store_sqlite.py); ran ALTER TABLE on VPS taris_vps; docker compose restart; added Taris - Notify Send N8N workflow + create-notify command to n8n_admin.py | 3 | 15 | claude-sonnet-4.6 | src/core/bot_db.py, src/core/store_postgres.py, src/core/store_sqlite.py, src/core/bot_config.py, src/release_notes.json, tools/n8n/n8n_admin.py | done |
| 13:00 UTC | Research: OpenClaw architecture analysis — how DEVICE_VARIANT gates behavior, what's underutilized (pgvector RAG, per-func LLM, MCP), 10 extension opportunities; then updated copilot-instructions.md + quick-ref.md with variant stacks quick-ref (libs, binaries, env vars, variant-aware coding rules); updated stacks.md version + LLM model table | 2 | 4 | claude-sonnet-4-6 | doc/quick-ref.md, .github/copilot-instructions.md, doc/architecture/stacks.md, doc/vibe-coding-protocol.md | done |
| 14:00 UTC | Translate OpenClaw research into TODO+docs: created doc/todo/28-openclaw-extensions.md (full spec §28 quick wins, §29 medium effort, §30 architecture flexibility with implementation tables, test IDs T200–T222); added §28–30 to TODO.md; added Extension Roadmap + unconnected assets table to openclaw-integration.md; bumped stacks.md + openclaw-integration.md versions | 3 | 3 | claude-sonnet-4.6 | doc/todo/28-openclaw-extensions.md (new), TODO.md, doc/architecture/openclaw-integration.md, doc/vibe-coding-protocol.md | done |
| 12:30 UTC | Fix v2026.4.63: guest menu inconsistency — same menu regardless of entry point (/start vs Back button); guest now sees Chat+Request Appointment+Calendar(read-only)+Profile+Help; fixed main_menu.yaml (digest visible to guests, meeting button missing, calendar missing); added _handle_guest_cal_event_detail read-only event view | 3 | 4 | claude-sonnet-4.6 | bot_access.py, main_menu.yaml, telegram_menu_bot.py, bot_calendar.py, bot_config.py, release_notes.json | done |
| 13:00 UTC | Fix v2026.4.64: Russian-language guests received English LLM responses (e.g. "Please specify what you would like to do!"). Root cause: {lang_instruction} was placed last in an all-English system prompt — LLM ignored it. Fix: moved {lang_instruction} to right after security_preamble in ALL role_system_prompts templates; strengthened Russian/German/English lang instructions with mandatory framing; persist detected language to DB on guest auto-registration via _set_reg_lang() | 2 | 2 | claude-sonnet-4.6 | src/prompts.json, src/telegram_menu_bot.py, src/core/bot_config.py, src/release_notes.json | done |
| 13:30 UTC | Feature v2026.4.65: New "Notifications to Users" agent in Agents menu — send Telegram notifications to contacts from Taris contact book; DB-stored templates (add/view/edit/delete); send flow (choose template / custom message → recipients all/filter → preview → confirm → send); Contacts extended with optional Telegram ID field | 3 | 3 | claude-sonnet-4.6 | src/features/bot_notify.py (new), src/features/bot_contacts.py, src/telegram_menu_bot.py, src/strings.json, src/core/bot_config.py, src/release_notes.json | done |
| 14:00 UTC | Fix v2026.4.66: guest conversation limit set to unlimited — GUEST_MSG_HOURLY_LIMIT/GUEST_MSG_DAILY_LIMIT defaults changed from 5/20 to 0 (unlimited); rate-limit check treats 0 as unlimited | 1 | 1 | claude-sonnet-4.6 | src/core/bot_config.py, src/telegram/bot_access.py, src/release_notes.json | done |
...
| 20:xx | Mark §25 + §26 done in TODO/DONE; create OBSOLETE.md (§17 Max UI, §18 ZeroClaw); fix install scripts (missing packages: pyTelegramBotAPI, fastembed, bcrypt, PyJWT, pymupdf, etc.) | 2 | 2 | claude-sonnet-4.6 | TODO.md, DONE.md, OBSOLETE.md (new), deploy/system-configs/taristation2/install.sh, deploy/system-configs/sintaition/install.sh, deploy/system-configs/vps/docker/deploy.sh | done |
| ~10:00 | /taris-update-doc: §1.1 RBAC configurable blocklist + admin Security Policy UI + T122 (15/15 PASS) + howto_admin/howto_bot updated + §0.1/§1.1 collapsed in TODO/DONE | 3 | 6 | claude-sonnet-4.6 | bot_security.py, bot_admin.py, telegram_menu_bot.py, strings.json, test_voice_regression.py, security.md, howto_admin.md, howto_bot.md, bot-code-map.md, TODO.md, DONE.md | done |
...
| 04:36 | Set LLM on SintAItion to gemma4:e4b (local Ollama, AMD ROCm GPU); smoke test OK | 1 | 1 | claude-sonnet-4.6 | SintAItion: bot.env (runtime only) | done |
...
| 21:43 | Fix TariStation2 Ollama crash-loop (10000+ restarts): port 11434 conflict between system-level ollama.service (PID 1351) and user-level duplicate; disabled user service; Ollama now responds; Taris chat restored | 2 | 4 | claude-sonnet-4.6 | TariStation2 runtime only (systemctl --user disable ollama) | done |
| 21:49 | Fix TariStation2 LLM timeout (90s): system Ollama runner CPU-saturated (106% CPU, concurrent queue); switched LLM_PROVIDER=openai in bot.env; verified 1.5s response; bot fully operational | 1 | 3 | claude-sonnet-4.6 | TariStation2 runtime only (~/.taris/bot.env) | done |
| 21:50 | Configure gemma4:e2b as default LLM on TariStation2: expose SintAItion Ollama on 0.0.0.0 (Tailscale), set OLLAMA_URL=http://100.112.120.3:11434 + OLLAMA_MODEL=gemma4:e2b + FALLBACK=openai; 2.8s response | 2 | 6 | claude-sonnet-4.6 | SintAItion runtime (/etc/systemd/system/ollama.service), TariStation2 runtime (~/.taris/bot.env) | done |
...
| 18:21 UTC | 16:05 UTC | 18:21 UTC | 136m | fix: docs UX + bot self-knowledge + voice retry robustness (v2026.4.5) | 3 bugs from error screenshots: docs_menu_title click-to-delete→click-to-manage; security_preamble rule 5 to fix 'I cant upload files' wrong answer; send_voice wrapped with retry; _tg_send_with_retry 5 retries exponential backoff | 2 | 1 | claude-sonnet-4.6 | src/core/bot_config.py, src/telegram/bot_access.py, src/features/bot_voice.py, src/strings.json, src/prompts.json, src/release_notes.json | done |
| 18:50 UTC | 16:22 UTC | 18:50 UTC | 148m | fix: SintAItion polling timeout spam + journal noise (v2026.4.6) | Diagnosed ReadTimeout flooding journal every ~90s; added infinity_polling outer restart guard + logger_level=DEBUG; reduced timeout 30s→25s; suppressed TeleBot logger via setLevel(CRITICAL) in bot_instance.py post-import; verified 0 ERROR-TeleBot on new process | 3 | 4 | claude-sonnet-4.6 | src/telegram_menu_bot.py, src/core/bot_config.py, src/core/bot_instance.py, src/release_notes.json | done |
| 19:09 UTC | 19:00 UTC | 19:09 UTC | 9m | fix: admin log view newest-first (v2026.4.7) | Reversed display order in _handle_admin_logs_show; trim from bottom not top; 401 PASS TS2 deployed | 1 | 1 | claude-sonnet-4.6 | src/telegram/bot_admin.py, src/core/bot_config.py, src/release_notes.json | done |
| 19:28 UTC | 19:10 UTC | 19:28 UTC | 18m | feat: system chat multi-turn history (v2026.4.8) | Added _system_history dict to bot_state; _handle_system_message uses ask_llm_with_history with full messages list; _save_system_history helper; history cleared on menu exit; 401 PASS TS2 deployed | 2 | 1 | claude-sonnet-4.6 | src/core/bot_state.py, src/telegram/bot_handlers.py, src/telegram_menu_bot.py, src/core/bot_config.py, src/release_notes.json | done |
| 20:17 UTC | 19:28 UTC | 20:17 UTC | 49m | fix: PostgreSQL shared docs in RAG + Ollama qwen3.5:0.8b on TS2 + deploy both targets (v2026.4.9) | Fixed 4 bugs in store_postgres (list_documents/search_fts/search_similar/get_chunks_without_embeddings all missing shared docs); re-indexed 136 system doc chunks with fastembed; pulled qwen3.5:0.8b; updated bot.env; deployed v2026.4.9 to TS2+SintAItion; 401 PASS | 3 | 3 | claude-sonnet-4.6 | src/core/store_postgres.py, src/core/bot_config.py, src/release_notes.json, doc/architecture/knowledge-base.md, doc/architecture/data-layer.md | done |
| 21:12 UTC | 18:52 UTC | 21:12 UTC | 140m | perf: fix menu response delay — num_threads=16 + background dispatch (v2026.4.10) | Diagnosed root cause (num_threads=2 + sync LLM/voice handlers blocking worker pool); measured API latency (send_message=229ms, edit=42ms); increased to 16 threads; dispatched chat/system/voice to daemon threads; added [PERF] timing logs; T91+T92; 402 PASS | 3 | 4 | claude-sonnet-4.6 | src/core/bot_instance.py, src/telegram_menu_bot.py, src/core/bot_config.py, src/release_notes.json, src/tests/test_voice_regression.py | done |
| 21:25 UTC | 21:13 UTC | 21:25 UTC | 12m | fix: missing openai package + lib audit | Checked all installed libs; found openai missing (LLM_PROVIDER=openai but ModuleNotFoundError); installed openai 2.30.0; added to requirements.txt + install_openclaw.sh | 1 | 1 | claude-sonnet-4.6 | deploy/requirements.txt, src/setup/install_openclaw.sh | done |
| 22:40 UTC | 22:20 UTC | 22:40 UTC | 20m | fix: stale-callback polling freeze + SIGKILL on restart (v2026.4.11) | Root cause: expired callbacks (>60s) crashed workers via ApiTelegramException→raise_exceptions()→exponential backoff 0.25→60s; bot silent for hours; fix: try/except on answer_callback_query; TimeoutStopSec 30→60s; T93 added; 2 PASS | 2 | 1 | claude-sonnet-4.6 | src/telegram_menu_bot.py, src/core/bot_config.py, src/services/taris-telegram.service, src/release_notes.json, src/tests/test_voice_regression.py | done |
| 23:13 UTC | 22:40 UTC | 23:13 UTC | 33m | fix: IPv6 stale-connection 48s menu hangs (v2026.4.12 + v2026.4.13) | Root cause: Fritz!Box IPv6 stateful firewall drops idle connections; bot's stale IPv6 connections hang READ_TIMEOUT=30s × retries = 128s; 3-layer fix: (1) TCP keepalive SO_KEEPALIVE/TCP_KEEPIDLE=30s via _KeepAliveAdapter injected into telebot.apihelper.session; (2) READ_TIMEOUT=10s; (3) _409Handler extended to suppress ReadTimeout/ConnectionError backoff | 3 | 2 | claude-sonnet-4.6 | src/core/bot_instance.py, src/core/bot_config.py, src/strings.json, src/release_notes.json | done |
| 23:40 UTC | 21:21 UTC | 23:40 UTC | 139m | fix: menu freeze from swap exhaustion — FW preload + tail_log + memory warn (v2026.4.14) | Analyzed 100s callback stalls via journald PERF logs; identified root cause: FasterWhisper small model (~460MB) preloaded at startup exhausted all 511MB swap on TariStation2 while Ollama (2GB) + Copilot (780MB) running; fixes: FASTER_WHISPER_PRELOAD=0 env var (saves 460MB), tail_log rewrite (subprocess tail vs 7.5MB readlines), /proc/meminfo startup warning; T94-T96 added; 408 PASS 0 FAIL | 3 | 1 | claude-sonnet-4.6 | src/core/bot_config.py, src/core/bot_logger.py, src/telegram_menu_bot.py, src/tests/test_voice_regression.py, src/release_notes.json | done |
| 12:00 UTC | | 11:30 UTC || 12:30 UTC | 60 | LLM perf benchmark + proposal on SintAItion (qwen3.5:latest) | benchmark script, run on remote, analyze results, write proposal | 3 | 5 | claude-sonnet-4.6 | tools/benchmark_llm_params.py, doc/llm-performance-proposal-2026-04-02.md, doc/howto_admin.md, doc/performance-report-2026-04-02.md | done |
| 16:24 UTC | 14:14 UTC | 16:24 UTC | 130m | feat: personal data context injection in LLM chat (v2026.4.15) | Calendar/notes/contacts stored in DB but not visible to LLM — added _calendar_context(), _notes_context(), _contacts_context() injected via _build_system_message(); T97 regression test; deployed TariStation2 | 3 | 1 | claude-sonnet-4.6 | src/telegram/bot_access.py, src/tests/test_voice_regression.py, src/core/bot_config.py, src/release_notes.json | done |
| 16:37 UTC | 16:35 UTC | 16:37 UTC | 2m | fix: context injection SQLite fallback (v2026.4.16 hotfix) | Root cause: STORE_BACKEND=postgres but user data in SQLite; _cal_load() pattern already has fallback; _notes/_contacts_context: try store first then get_db() SQLite; calendar also shows recent past when no upcoming; deployed+committed e25816e | 2 | 1 | claude-sonnet-4.6 | src/telegram/bot_access.py | done |
| 16:50 UTC | 14:38 UTC | 16:50 UTC | 132m | perf: SintAItion LLM+STT tuning (v2026.4.17) | power profile perf (CPU 2.5→5.2GHz), OLLAMA_NUM_CTX=8192 (was 262144), KV_CACHE_TYPE=q8_0, FASTER_WHISPER_THREADS=12; LLM improved: 18.7s→11.2s (40%); all deployed+committed | 2 | 1 | claude-sonnet-4.6 | src/core/bot_config.py, src/core/bot_llm.py, src/release_notes.json | done |
| 21:43 UTC | 19:36 UTC | 21:43 UTC | 127m | perf: full SintAItion optimization stack (v2026.4.18) | streaming chat (first text in 1-2s), LOCAL_MAX_TOKENS=256 (was 512), keep_alive in API body, vm.swappiness=10, ollama service NUM_PARALLEL=1+MAX_LOADED=1+KEEP_ALIVE=24h; all deployed+committed | 3 | 1 | claude-sonnet-4.6 | src/core/bot_config.py, src/core/bot_llm.py, src/telegram/bot_handlers.py, src/release_notes.json | done |
| 21:52 UTC | 21:43 UTC | 21:52 UTC | 9m | perf: STT optimizations — thread cap fix, timestamps off, VAD pad (v2026.4.19) | Fix: FASTER_WHISPER_THREADS explicit value was silently capped at 8 regardless of setting; without_timestamps=True skips unused computation; VAD speech_pad_ms=200 (was 400ms); SintAItion now runs threads=12 confirmed in logs; deployed+committed | 2 | 1 | claude-sonnet-4.6 | src/features/bot_voice.py, src/core/bot_config.py, src/release_notes.json | done |
| 22:34 UTC | 20:22 UTC | 22:34 UTC | 132m | analysis: last conversations + perf fix history cap + embedding pre-warm (v2026.4.20) | Analyzed SintAItion logs: LLM=20.4s (history=15 + personal context = 3000+ tokens to Ollama), RAG=2018ms cold-start (fastembed model missing), TariStation2 LLM=1.9-3.0s (OpenAI cloud); Fixed: CONVERSATION_HISTORY_MAX=8 (saves ~7s LLM prefill), EmbeddingService pre-warmed at startup, fastembed ONNX model downloaded (5x faster, backend=fastembed confirmed in logs); deployed+committed | 3 | 1 | claude-sonnet-4.6 | src/telegram_menu_bot.py, src/core/bot_config.py, src/release_notes.json | done |
| 22:38 UTC | 20:35 UTC | 22:38 UTC | 3m | perf: SintAItion switch to OpenAI primary LLM (v2026.4.21) | Live log comparison: SintAItion 16.7s vs TariStation2 9.1s — entire gap was LLM (14.3s ollama vs 1.8s openai); switched LLM_PROVIDER=openai + LLM_FALLBACK_PROVIDER=ollama on SintAItion; smoke test 0.46s (31x faster); deployed+committed | 1 | 1 | claude-sonnet-4.6 | src/core/bot_config.py, src/release_notes.json, ~/.taris/bot.env@SintAItion | done |

| 22:56 UTC | 22:47 UTC | 22:56 UTC | 9m | fix: LLM silent fallback to Ollama — invalid active_model.txt (v2026.4.22) | Root cause found: active_model.txt contained 'openai/gpt-4.1-mini' (invalid OpenAI model name) → HTTP 400 → silent fallback to Ollama → 14s latency despite LLM_PROVIDER=openai; Fix: reset active_model.txt to gpt-4o-mini + get_active_model() now strips provider/ prefix to prevent recurrence + improved HTTP 4xx logging shows model name; LLM now 0.96s | 2 | 1 | claude-sonnet-4.6 | src/core/bot_llm.py, src/core/bot_config.py, src/release_notes.json | done |

| 23:09 UTC | 22:55 UTC | 23:09 UTC | 14m | fix: voice LLM silently using Ollama via per_func chat override (v2026.4.23) | Root cause: ask_llm_with_history() defaulted to use_case='chat'; per_func['chat']='ollama' on SintAItion → all voice LLM calls went to slow Ollama (12-15s). TariStation2 had per_func['chat']='' → OpenAI (1.8s). Log showed 'provider=openai' (LLM_PROVIDER const, not actual) — 100% misleading. Fix: use_case='voice' + accurate log; v2026.4.23 deployed SintAItion | 3 | 1 | claude-sonnet-4.6 | src/features/bot_voice.py, src/core/bot_config.py, src/release_notes.json | done |

| 21:22 UTC | 21:22 UTC | 21:53 UTC | 31m | deploy: sync TariStation2 service to v2026.4.23 + verify SintAItion all fixes present | Compared deployed vs source: SintAItion already had all 5 fixes (v2026.4.19-4.23); TariStation2 service was at v2026.4.16. Deployed bot_config.py, bot_llm.py, bot_voice.py, telegram_menu_bot.py, migrate_reembed.py, bot_handlers.py(streaming) to ~/.taris/. Tests: SintAItion 402 PASS 0 FAIL; TariStation2 402 PASS 0 FAIL 1 WARN (fallback_silence pre-existing: no STT_LANG=ru in TS2 bot.env) | 2 | 3 | claude-sonnet-4.6 | ~/.taris/core/bot_config.py, ~/.taris/core/bot_llm.py, ~/.taris/features/bot_voice.py, ~/.taris/telegram_menu_bot.py, ~/.taris/setup/migrate_reembed.py, ~/.taris/telegram/bot_handlers.py | done |
...
| 12:21 UTC | 10:00 UTC | 12:21 UTC | 141m | feat: PicoClaw→Taris rename + configurable male TTS voice per user | Renamed user-facing PicoClaw→Taris across config/admin/ui. Added male voice (dmitri-medium.onnx) as per-user toggle in Profile (Telegram + Web UI). DB migration: voice_male column. _piper_model_path per-user. BOT_VERSION 2026.4.25. Downloaded dmitri model to TariStation2. Deployed + verified. | 3 | 2 | claude-sonnet-4.6 | src/core/bot_config.py, src/core/bot_db.py, src/core/store_sqlite.py, src/core/store_postgres.py, src/features/bot_voice.py, src/telegram/bot_admin.py, src/telegram/bot_handlers.py, src/telegram_menu_bot.py, src/screens/profile.yaml, src/screens/screen.schema.json, src/strings.json, src/bot_web.py, src/web/templates/profile.html, src/ui/bot_ui.py, src/voice_assistant.py, src/release_notes.json | done |

| 17:08 UTC | 17:08 UTC | 17:12 UTC | 4m | fix: voice_male column missing in PostgreSQL voice_opts | Add voice_male to PG schema DDL + migration | 1 | 1 | claude-sonnet-4.6 | src/core/store_postgres.py | done |
| 17:26 UTC | 17:26 UTC | 17:38 UTC | 12m | fix: empty note render + voice admin Markdown injection + T98+T99 | Two bugs from journal; T98 empty MarkdownBlock guard; T99 admin info backticks | 2 | 1 | claude-sonnet-4.6 | src/ui/render_telegram.py, src/features/bot_voice.py, src/tests/test_voice_regression.py, src/core/bot_config.py, src/release_notes.json, doc/test-suite.md | done |
| 17:39 UTC | 17:39 UTC | 17:46 UTC | 7m | fix: document detail crash — Postgres created_at is datetime not string | Detected via python3 debug; fixed hasattr(strftime) guard; T100 added | 2 | 1 | claude-sonnet-4.6 | src/features/bot_documents.py, src/tests/test_voice_regression.py, src/core/bot_config.py, src/release_notes.json, doc/test-suite.md | done |
| 18:00 UTC | 18:00 UTC | 18:11 UTC | 11m | fix: note open crash for 0-byte note files | _handle_note_open empty guard + render_screen path logging + T101 | 2 | 1 | claude-sonnet-4.6 | src/telegram/bot_handlers.py, src/ui/render_telegram.py, src/tests/test_voice_regression.py, src/core/bot_config.py, src/release_notes.json, doc/test-suite.md | done |

| 18:11 UTC | 18:11 UTC | 18:55 UTC | 44m | fix: notes stored to wrong path (str(chat_id) vs UUID); orphaned note recovery | Investigated orphaned UUID dirs; found accounts.json linkage; migrated content; fixed _notes_storage_dir in store_postgres; T102 | 3 | 1 | claude-sonnet-4.6 | src/core/store_postgres.py, src/tests/test_voice_regression.py, src/core/bot_config.py, src/release_notes.json, doc/test-suite.md, doc/vibe-coding-protocol.md | done |

| 19:26 UTC | 19:26 UTC | 19:27 UTC | 1m | bot_db Layer 2 → Postgres migration: system_settings JSON, store delegation for history/LLM/prefs/summaries/security | 7 files edited; new DDL tables in postgres; new methods in both stores; bot_state/contacts/dev refactored | 4 | 1 | claude-sonnet-4.6 | src/core/bot_db.py, src/core/store_base.py, src/core/store_postgres.py, src/core/store_sqlite.py, src/core/bot_state.py, src/features/bot_contacts.py, src/features/bot_dev.py | done |

| 19:34 UTC | 19:25 UTC | 19:45 UTC | 20m | Phase 2: all user data to DB — web_accounts, mail_creds, link_codes | store_postgres+sqlite 12 new methods; bot_auth DB-backed + accounts.json migration; bot_state link codes to DB; mail_creds store-primary; bot_web store lookup; T103-T105 | 4 | 1 | claude-sonnet-4.6 | src/core/store_postgres.py, src/core/store_sqlite.py, src/security/bot_auth.py, src/core/bot_state.py, src/features/bot_mail_creds.py, src/bot_web.py, src/tests/test_voice_regression.py, doc/architecture/data-layer.md | done |

| 20:34 UTC | 20:20 UTC | 20:52 UTC | 32m | SQLite fully eliminated on OpenClaw — Postgres-only, data migration, T106 | bot_access/users/calendar/bot_db/telegram_menu_bot SQLite fallbacks removed; migrate_sqlite_to_postgres.py; 316 rows migrated; link code revocation fixed; T106 test | 4 | 1 | claude-sonnet-4.6 | src/telegram/bot_access.py, src/telegram/bot_users.py, src/features/bot_calendar.py, src/core/bot_db.py, src/telegram_menu_bot.py, src/core/bot_state.py, src/setup/migrate_sqlite_to_postgres.py, src/tests/test_voice_regression.py, doc/architecture/data-layer.md | done |
| 21:28 UTC | | 21:23 UTC | 21:28 UTC | 5m | fix: LLM_FALLBACK_PROVIDER missing in ask_llm_with_history + dict_row list_active_chat_ids | 2 | 3 | claude-sonnet-4.6 | src/core/bot_llm.py, src/core/store_postgres.py, src/tests/test_voice_regression.py, src/core/bot_config.py, src/release_notes.json | done |

| 20:03 UTC | 20:03 UTC | 18:46 UTC | Check STT/TTS feasibility on VPS aarch64: benchmarked faster-whisper (tiny RTF=0.14, base RTF=0.23) + Piper TTS (RTF=0.12-0.22); all ARM64 wheels verified; created install_voice.sh | 2 | 4 | claude-sonnet-4.6 | doc/vps-taris-deployment-analysis.md, deploy/system-configs/vps/install_voice.sh | done |
| 20:10 UTC | 7m | check old Copilot session results + complete pending tasks (commit vibe-coding-protocol, full test suite v2026.4.33) | committed vibe-coding-protocol.md; PASS 446 FAIL 4 (pre-existing) SKIP 22 WARN 1 | 1 | 3 | claude-sonnet-4.6 | doc/vibe-coding-protocol.md | done |
...
| 22:44 UTC | 22:38 UTC | 22:44 UTC | 6m | network speed "not available" — add net ifaces+speed tools to _HOST_CTX | _build_host_ctx: UP ifaces, default gw, speed tool list (speedtest/iperf3/fallback) | 2 | 1 | claude-sonnet-4.6 | src/telegram/bot_handlers.py, src/core/bot_config.py, src/release_notes.json | done |

| 07:15 UTC | 06:45 UTC | 08:15 UTC | 90m | SintAItion full migration: SQLite→PostgreSQL + v2026.4.36 deploy + full test suite + perf benchmarks | Phase 0-13: backup, install PG, fix notes migration (empty content from .md), deploy, migrate 316 rows (3 users/22 cal/94 history/12 notes/2 contacts/3 docs), fix store_sqlite rag_stats def missing, fix bot_state conversation_summaries ref, fix 3 test crash bugs, fix benchmark CWD; 461 PASS 0 FAIL 20 SKIP | 5 | 15 | claude-sonnet-4.6 | src/setup/migrate_sqlite_to_postgres.py, src/core/store_sqlite.py, src/core/bot_state.py, src/tests/test_voice_regression.py, tools/benchmark_menus.py | done |

| 09:10 UTC | 07:10 UTC | 09:35 UTC | 145m | Post-migration test updates + UI tests | Branch rules updated (taris-openclaw→master, 6 files); T111-T113 added (migrate structure, contacts parity, postgres live data); T114 added (admin page datetime fix); UI tests: 61/61 PASS after fixing bot_web.py admin_page str() wrap bug | 2 | 4 | claude-sonnet-4.6 | src/tests/test_voice_regression.py, doc/test-suite.md, src/bot_web.py, AGENTS.md, .github/instructions/openclaw.instructions.md, .github/skills/taris-deploy-openclaw-target/SKILL.md | done |
| 19:48 UTC | 19:48 UTC | 20:15 UTC | 27m | Fix [BOT CAPABILITIES] literal tag in LLM replies + add user/admin guide as RAG docs with admin-only filtering | 1. prompts.json rule 5: instruct LLM to enumerate capabilities directly, never print block markers; 2. load_system_docs.py: is_shared=2 for admin guide (admin-only), is_shared=1 for user guide; 3. store_sqlite/postgres search_fts+search_similar: is_admin param filters is_shared=2 docs; 4. bot_rag.retrieve_context: propagates is_admin; 5. bot_access._docs_rag_context: passes _is_admin(chat_id); v2026.4.37 | 3 | 2 | claude-sonnet-4.6 | src/prompts.json, src/setup/load_system_docs.py, src/core/store_sqlite.py, src/core/store_postgres.py, src/core/store_base.py, src/core/bot_rag.py, src/telegram/bot_access.py, src/core/bot_config.py, src/release_notes.json | done |
| 22:03 UTC | | | | Regression tests for above fixes | T90 fix (stale is_shared=1 check to shared_level/is_shared=2); T115 (capabilities tag fix); T116 (admin-only RAG stack); update doc/test-suite.md | 2 | 1 | claude-sonnet-4.6 | src/tests/test_voice_regression.py, doc/test-suite.md | done |
| 22:05 UTC | | 22:21 UTC | 16 min | §19.4/§25.6 open items: implement MCP admin UI + TODO cleanup + §27 Multimodal RAG | 1. Mark §19.4 items done (migration/pgvector/visible_variants already impl); 2. Mark §25.6 sqlite-vec done; 3. Implement MCP admin UI (bot_admin.py handlers + bot_mcp_client DB override + i18n strings + telegram_menu_bot dispatch); 4. Add §27 Multimodal RAG topic; 5. Version bump 2026.4.38 | 2 | 4 | claude-sonnet-4.6 | src/telegram/bot_admin.py, src/core/bot_mcp_client.py, src/telegram_menu_bot.py, src/strings.json, src/core/bot_config.py, src/release_notes.json, TODO.md | done |
| 13:30 UTC | | | | fix: Postgres vec_embeddings DDL crash when pgvector unavailable (v2026.4.39) | 1. Remove embedding vector(384) column and HNSW index from base vec_embeddings DDL in _SCHEMA_SQL; 2. Add conditional ALTER TABLE migration in _init_schema — adds embedding+HNSW only when _has_vec=True; 3. vec_embeddings now always created (FTS path works without pgvector); 4. Fixes UndefinedTable crash in upsert_chunk_text / load_system_docs.py --force; commit e3cd34e | 2 | 3 | claude-sonnet-4.6 | src/core/store_postgres.py, src/core/bot_config.py, src/release_notes.json | done |
...
| 18:05 UTC | Implement TODO §28-29.3: per-user Ollama model, N8N event router, CRM sync, skill rendering, Ollama pull UI, 8 new tests T200-T207 | 4 | ~10 | claude-opus-4.6 | bot_config.py, bot_llm.py, bot_admin.py, bot_handlers.py, bot_contacts.py, bot_n8n.py, bot_voice.py, bot_web.py, telegram_menu_bot.py, render_telegram.py, strings.json, release_notes.json, .env.example, test_voice_regression.py | done |
| 19:55 UTC | | 19:48 UTC | | 20:00 UTC | 12 | Merge master→feature/guest-user-prompt-templates, deploy v2026.4.68 to VPS Docker, run full tests (613 PASS, 12 pre-existing FAIL) | 1 | 4 | claude-sonnet-4.6 | test_voice_regression.py (fix), doc/vibe-coding-protocol.md | done |
...
| 04:55 UTC | T200-T215 Remote KB agent E2E tests; fix _cb_is_open() global scope bug; fix n_chunks format-string bug in strings.json; update doc/test-suite.md | 3 | ~8 | claude-sonnet-4.6 | src/tests/test_remote_kb.py (new), src/core/bot_mcp_client.py, src/strings.json, doc/test-suite.md | done |
| 02:58 UTC | Remove legacy systemd taris-telegram/voice/web service files from VPS-Supertaris; update AGENTS.md, copilot-instructions.md, deployment.md, SKILL.md to remove conflict warnings | 1 | 1 turn | claude-sonnet-4.6 | AGENTS.md, .github/copilot-instructions.md, doc/architecture/deployment.md, .github/skills/taris-deploy-openclaw-target/SKILL.md | done |
| 03:10 UTC | Fix 4 KB upload/error bugs; add remote_kb_op_fail+remote_kb_upload_empty strings; add T216-T219 error-path E2E tests; 22/22 pass | 3 | 4 turns | claude-sonnet-4.6 | src/features/bot_remote_kb.py, src/strings.json, src/tests/test_remote_kb.py, doc/test-suite.md | done |
| 13:00 UTC | Fix N8N KB Ingest workflow fully end-to-end: resolved 5 bugs (SplitInBatches v3→v1, Ollama 0.0.0.0, HTTP body format, chunk loop break, n8n_user sequence permission); verified chunks in taris_kb DB with 384-dim embeddings | 4 | ~30 | claude-sonnet-4.6 | src/n8n/workflows/KB - Ingest.json | done |
| 16:34 UTC | Deploy KB list/search/delete direct-psycopg fix to VPS; add T220–T224 integration tests; fix n_chunks key mismatch + T205 bot_config reload bug; 27/27 pass on VPS-Supertaris | 3 | 5 turns | claude-sonnet-4.6 | src/features/bot_remote_kb.py, src/tests/test_remote_kb.py, doc/test-suite.md | done |
| 18:41 UTC | KB upload: multi-file support — session stays alive after each file, Done inline button, finish_upload() API, remote_kb_upload_done callback; strings updated (ru/en/de); 22/22 offline pass | 2 | 3 turns | claude-sonnet-4.6 | src/features/bot_remote_kb.py, src/strings.json, src/telegram_menu_bot.py, src/tests/test_remote_kb.py | done |
| 16:30 UTC | Fix KB list_documents: N8N MCP SSE incompatible with Python mcp lib (RemoteProtocolError); bypass with direct psycopg + pgvector; add KB_PG_DSN config; deployed to VPS, verified 2 docs returned instantly | 3 | 5 turns | claude-sonnet-4.6 | src/core/bot_mcp_client.py, src/core/bot_config.py | done |
| 10:55 UTC | | 10:45 UTC | | 10:55 UTC | 10m | Add mandatory E2E testing on deployed targets to instructions and skills | Update 4 files: bot-coding instructions + 3 skill files | 2 | 1 | claude-sonnet-4-5 | bot-coding.instructions.md, taris-deploy-openclaw-target/SKILL.md, taris-deploy-to-target/SKILL.md, taris-test-software/SKILL.md | done |
| | | | | | Fix CAMP/CS test isolation: _reload_bot_n8n() force-reload helper, align T_CS API to current bot_content (on_post_action, _do_generate_plan/post, on_pub_config_input, urllib.request.urlopen publish); 185/185 offline PASS | 3 | 3 | claude-sonnet-4.6 | src/tests/test_campaign.py, src/tests/test_content_strategy.py | done |
