...
Designed to compare Gemma 4, Qwen3.5 series against existing Qwen3 / Qwen2 models.

Usage (run on the target machine — needs Ollama running):
    python3 src/tests/llm/benchmark_ollama_models.py
    python3 src/tests/llm/benchmark_ollama_models.py --model gemma4:e4b
...
def _get_model_info(model: str) -> dict:
    ...
    try:
        return _ollama_request("/api/show", {"model": model}, timeout=15)
    except Exception:
        return {}
...
def _run_prompt(model: str, prompt: str, options: dict) -> dict:
    ...
    Disables thinking mode for qwen3/qwen3.5/gemma4 models (they consume all
    tokens in <think> blocks otherwise, returning empty responses).
...
def benchmark_model(model: str, prompt_keys: list[str], verbose: bool = False) -> ModelResult:
    ...
    if info and verbose:
        ...
        print(f"  Family: {details.get('family', 'n/a')}, "
              f"Params: {details.get('parameter_size', 'n/a')}, "
              f"Quant: {details.get('quantization_level', 'n/a')}")
...
def print_lang_summary(results: list[ModelResult], langs: list[str]):
    """Per-language quality matrix: models x languages."""
    if len(langs) < 2:
        return
...
def main():
    parser = argparse.ArgumentParser(description="Ollama LLM model benchmark -- Gemma 4, Qwen, 4 languages")
