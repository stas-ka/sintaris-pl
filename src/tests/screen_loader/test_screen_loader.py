...
def _user(role: str = "user", lang: str = "en", variant: str = "picoclaw") -> UserContext:
    return UserContext(user_id="u1", chat_id=123, lang=lang, role=role, variant=variant)
...
class TestIsVisible:
    ...
    def test_visible_variants_match(self):
        w = {"visible_variants": ["openclaw"]}
        assert _is_visible(w, _user(variant="openclaw")) is True

    def test_visible_variants_no_match(self):
        w = {"visible_variants": ["openclaw"]}
        assert _is_visible(w, _user(variant="picoclaw")) is False

    def test_visible_variants_multiple(self):
        w = {"visible_variants": ["picoclaw", "openclaw"]}
        assert _is_visible(w, _user(variant="picoclaw")) is True
        assert _is_visible(w, _user(variant="openclaw")) is True

    def test_visible_variants_combined_with_roles(self):
        # Both role AND variant must match
        w = {"visible_roles": ["admin"], "visible_variants": ["openclaw"]}
        assert _is_visible(w, _user(role="admin", variant="openclaw")) is True
        assert _is_visible(w, _user(role="admin", variant="picoclaw")) is False
        assert _is_visible(w, _user(role="user", variant="openclaw")) is False
...
class TestMainMenuYaml:
    ...
    def test_main_menu_user_sees_8_widgets(self):
        """User should see 9 widgets (12 total minus 3 admin-only rows)."""
        self._skip_if_missing()
        screen = load_screen(self.SCREEN_PATH, _user("user"), variables={}, t_func=_t)
        assert len(screen.widgets) == 9

    def test_main_menu_admin_sees_11_widgets(self):
        """Admin should see all 12 widgets (includes Agents + Digest buttons)."""
        self._skip_if_missing()
...
class TestAdminMenuYaml:
    ...
    def test_admin_menu_yaml_loads_with_empty_badge(self):
        """File loads without error; picoclaw admin sees 14 widgets (openclaw row hidden)."""
        self._skip_if_missing()
        screen = load_screen(
    ...
    def test_admin_menu_openclaw_sees_extra_button(self):
        """OpenClaw admin sees 15 widgets (14 standard + OpenClaw gateway button)."""
        self._skip_if_missing()
        screen = load_screen(
