...
- **Agent/service fix → migrate existing data — MANDATORY:** When fixing or rewriting any agent, MCP client, KB pipeline, data store adapter, or data-producing service, you MUST migrate all existing data produced by the old implementation to be compatible with the new one. Steps: (1) identify all tables/collections/files written by the component; (2) write and run a migration script or SQL statement; (3) verify row counts and spot-check a sample before and after; (4) confirm with the user before running DDL or bulk updates on production. Never leave stale/incompatible data from the old implementation in place after a fix is deployed.
